...
def _gini(x: np.ndarray) -> float:
    ...
    # pre-process (values start at 0+eps)
    if x.min() < 0: x -= x.min()
    x = x.clip(1e-8)
    x = np.sort(x.flatten())
    ...
    g = ((2 * idx - len(x)  - 1) * x).sum() / (len(x) * x.sum())
...
def shap_interactions(
    ...
    for f, d, s in zip(feature_names, feature_data, feature_shaps):

        # sort data
        p = np.argsort(d)
        d, s = d.copy()[p], s.copy()[p]

        # get optimal split that minimizes gini impurity
        best_split = None
        best_score = np.inf
        for i in range(1, len(d)):
            split = (d[i-1] + d[i]) / 2
            y1 = (np.sign(s[d < split]) + 1) / 2
            y2 = (np.sign(s[d >= split]) + 1) / 2
            g1 = _gini(y1) if len(y1) > 1 else 0
            g2 = _gini(y2) if len(y2) > 1 else 0
            score = g1 + g2
            if score < best_score:
                best_split = split
                best_score = score
        individual_split.append(best_split)

        # plot feature vs shap value with optimal split
        ax = fig.add_subplot(gs[count, 0])
        ax.scatter(d, s, color='k', label='Training values')
        ax.axvline(x=best_split, color='k', linestyle='--', 
                label=f'Optimal split: {best_split:.2f}', linewidth=2)
    ...
    # get optimal feature splits for pairs of features (quadrants)
    p1 = np.argsort(d1)
    p2 = np.argsort(d2)
    interaction_split = None
    best_score = np.inf
    for i in tqdm(range(1, len(p1))):
        for j in range(1, len(p2)):
            split1 = (d1[p1[i-1]] + d1[p1[i]]) / 2
            split2 = (d2[p2[j-1]] + d2[p2[j]]) / 2
            y11 = (np.sign(interactions[(d1 < split1) & (d2 < split2)]) + 1) / 2
            y12 = (np.sign(interactions[(d1 < split1) & (d2 >= split2)]) + 1) / 2
            y21 = (np.sign(interactions[(d1 >= split1) & (d2 < split2)]) + 1) / 2
            y22 = (np.sign(interactions[(d1 >= split1) & (d2 >= split2)]) + 1) / 2
            g11 = _gini(y11) if len(y11) > 1 else 0
            g12 = _gini(y12) if len(y12) > 1 else 0
            g21 = _gini(y21) if len(y21) > 1 else 0
            g22 = _gini(y22) if len(y22) > 1 else 0
            score = g11 + g12 + g21 + g22
            if score < best_score:
                interaction_split = (split1, split2)
                best_score = score
    ...
    ax.axvline(x=interaction_split[0], color='k', linestyle='--', linewidth=2, 
            label=f'{rename[f1]} split: {interaction_split[0]:.2f}')
    ax.axhline(y=interaction_split[1], color='k', linestyle='-.', linewidth=2, 
            label=f'{rename[f2]} split: {interaction_split[1]:.2f}')
    ...
    return individual_split, interaction_split
